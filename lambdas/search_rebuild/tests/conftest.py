"""Test harness for the search-rebuild Lambda (Story 4.3, ATDD).

- Adds `lambdas/` to sys.path so `import search_rebuild` resolves to
  `lambdas/search_rebuild/` (in the Lambda zip the package sits at the zip
  root, same layout as `shared/`).
- Registers `lambdas/_shared` as the `shared` package (same alias as
  `lambdas/search_consumer/tests/conftest.py`) so the handler's
  `from shared import ...` imports resolve identically in tests and in the
  Lambda runtime.
"""
