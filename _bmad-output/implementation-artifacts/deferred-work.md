...
## Deferred from: code review of story-4.1 (2026-08-24)

- source_spec: `_bmad-output/implementation-artifacts/spec-4-1-search-consumer-indexing-processed-videos.md`
  summary: The search leg's deployed wiring has no repeatable automated integration coverage (zip source-block completeness, rule-pattern <-> publisher DetailType agreement, tf env names <-> handler reads).
  evidence: Review demonstrated all five CI stages stay green with the shared/events.py zip source block deleted or the rule pattern staled — unit tests use fakes and never see the zip or real wiring, and tests/integration/ covers only the other legs. Story 4.4 SM-1's Bruno collection exercises this path end-to-end, but a tests/integration/test_search_leg.py mirroring test_history_leg.py (or a zip-content drift check) would catch regressions earlier.
