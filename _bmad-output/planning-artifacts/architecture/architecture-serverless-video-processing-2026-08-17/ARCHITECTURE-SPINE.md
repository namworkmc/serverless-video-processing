...
| Data & formats (ids, dates, error shapes, envelopes) | `videoId` UUID minted at ingress (AD-6); `eventId` deterministic from `(videoId, status)`; timestamps ISO-8601 UTC strings; event payloads JSON carrying `eventId` + `schemaVersion` + `detail`; SQS-delivered events arrive as `Records[].body` = JSON-stringified EventBridge envelope — consumers unwrap `body → detail`; HTTP error body `{"error": ...}` (AD-7); **the gateway delivers non-text bodies (incl. multipart) base64** (`isBase64Encoded: true`, floci >= 1.7.0 PR #2203, matches real AWS) — the upload handler decodes, then parses multipart itself (supersedes the 1.6.0-era "raw multipart" spike finding; the 1.6.0 raw delivery corrupted high-byte payloads — epic-1 retro AI-1, resolved by the 1.7.0 bump) |
